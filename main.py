import requests,user_agent,json,flask
...
def index():
    return "<h1>WELCOME TO THE API  SIDRAELEZZ TELEGRAM : @MMPMMMM</h1>"


@app.route('/check-live/yahoo/', methods=['GET'])
def yahoo():
	
	eml = str(request.args.get('email'))
	...
	if ('"yid"') in response:
		return jsonify(check=False, email= eml, Telegram="@MMPMMMM") 
		
	elif ('"birthDate"') in response:
		
		return jsonify(check=True, email= eml, Telegram="@MMPMMMM") 
